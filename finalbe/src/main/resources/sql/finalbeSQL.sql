...
-- Schema ssafyweb
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `happy_house` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci ;
USE `happy_house` ;

-- -----------------------------------------------------
-- Table `ssafyweb`.`members`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `happy_house`.`members` ;

CREATE TABLE IF NOT EXISTS `happy_house`.`members` (
...
insert into members
values ('ssafy', '김싸피', 'ssafy', 'ssafy@ssafy.com', now(), null);

insert into members
values ('admin', '관리자', 'admin', 'admin@ssafy.com', now(), null);


-- -----------------------------------------------------
-- Table `ssafyweb`.`board`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `happy_house`.`board` ;

CREATE TABLE IF NOT EXISTS `happy_house`.`board` (
...
create table `happy_house`.`comment`(
...
create table `file`(
    ...
    saved_path varchar(100) not null unique,
    foreign key(`article_no`) references board(`article_no`) -- 글이 존재해야 파일을 업로드할 수 있음 
);
...
-- news data information 2022-11-02
